...
- LOOP-018 为 `IN_PROGRESS`：已替换静态 Agent Lab，新增服务端 Experiment contracts、SQLite append-only definition/event、actor-scoped catalog/API、Graph Backtest/Walk-Forward job 复用与 Candidate-only 工作台；`check`、336/336 tests、Web build 和 diff-check 通过。真实 Chrome 成功打开实验场，但 Chrome 页面 DOM 通道超时重置，未能完成创建/Evidence/replay/双尺寸/Console 安全验收；不标记 M3 完成。下一步执行 [`LOOP-019`](loop-prompts/loop-019-m3-experiment-lab-continuation.md) 继续 M3，禁止进入 M4 或要求人工验收。
