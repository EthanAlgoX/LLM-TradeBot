...
- 下一步执行唯一编号 [`LOOP-010`](loop-prompts/loop-010-m2-manual-evidence-and-doc-convergence-v1.md)（M2 用户手工证据关闭与规划文档收敛，不进入 M3）。Agent 禁止调用任何浏览器控制工具；必须发送验收表并等待用户手工 Chrome 反馈。全部通过后关闭 M2、收敛过期快照并生成 LOOP-011（M3）。
