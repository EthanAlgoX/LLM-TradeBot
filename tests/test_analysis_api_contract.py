...
class AnalysisApiContractTestCase(unittest.TestCase):
    ...
    def test_get_task_list_includes_analysis_phase_and_skills(self) -> None:
        ...
        self.assertEqual(response.tasks[0].strategy_version_id, 57)
        self.assertEqual(response.tasks[0].created_at, "2026-04-10T12:00:00")
        self.assertEqual(response.tasks[0].started_at, "2026-04-10T12:00:01")

    def test_get_task_list_merges_durable_strategy_report_history(self) -> None:
        if get_task_list is None:
            self.skipTest("analysis endpoint helpers unavailable in this environment")

        queue = MagicMock()
        queue.list_all_tasks.return_value = []
        queue.get_task_stats.return_value = {"total": 0, "pending": 0, "processing": 0}
        database = MagicMock()
        database.list_strategy_analysis_runs.return_value = [{
            "query_id": "saved-research-run",
            "code": "600519",
            "name": "贵州茅台",
            "report_type": "detailed",
            "strategy_version_id": 57,
            "submitted_at": "2026-08-18T09:00:00",
            "started_at": "2026-08-18T09:00:01",
            "completed_at": "2026-08-18T09:01:00",
            "created_at": "2026-08-18T09:01:00",
        }]

        with (
            patch("api.v1.endpoints.analysis.get_task_queue", return_value=queue),
            patch("src.storage.DatabaseManager.get_instance", return_value=database),
        ):
            response = get_task_list(status=None, limit=20, strategy_version_id=57)

        self.assertEqual(response.total, 1)
        self.assertEqual(response.tasks[0].task_id, "saved-research-run")
        self.assertEqual(response.tasks[0].strategy_version_id, 57)
        self.assertEqual(response.tasks[0].completed_at, "2026-08-18T09:01:00")
