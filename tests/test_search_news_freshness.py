...
class SearchNewsFreshnessTestCase(unittest.TestCase):
    ...
    def test_strategy_can_lock_news_search_to_one_configured_provider(self) -> None:
        service = SearchService(
            bocha_keys=["dummy_key"],
            searxng_public_instances_enabled=False,
            news_max_age_days=3,
        ...
        skipped = SimpleNamespace(
            is_available=True,
            name="Bocha",
            search=MagicMock(return_value=_response([_result("不应使用", datetime.now().date().isoformat())])),
        )
        selected = SimpleNamespace(
            is_available=True,
            name="Tavily",
            search=MagicMock(return_value=_response([_result("目标来源", datetime.now().date().isoformat())])),
        )
        service._providers = [skipped, selected]

        response = service.search_stock_news("600519", "贵州茅台", provider_name="Tavily")

        self.assertTrue(response.success)
        skipped.search.assert_not_called()
        selected.search.assert_called_once()
