...
const createSchema = z.object({ category: AgentCategorySchema, payload: z.unknown() }).strict();
const versionSchema = z.object({ parentVersionId: z.string().min(1), parentFingerprint: z.string().regex(/^sha256:[a-f0-9]{64}$/), payload: z.unknown() }).strict();
  ...
  async handle(request: Request): Promise<Response> {
    try {
      ...
      if (request.method === "GET" && path === "/api/orchestration/agents") {
        const keys = [...url.searchParams.keys()]; if (keys.some((key) => key !== "category") || url.searchParams.getAll("category").length > 1) throw new AgentDefinitionError("QUERY_INVALID");
        const category = url.searchParams.get("category"); return response({ data: this.service.list(actor.actorId, category ? AgentCategorySchema.parse(category) : undefined), runtimeApplied: false, exchangeWriteAllowed: false });
      }
      if (request.method === "POST" && path === "/api/orchestration/agents") { const body = createSchema.parse(await request.json()); return response({ data: this.service.create(actor.actorId, body.category, body.payload) }, 201); }
      const versions = path.match(/^\/api\/orchestration\/agents\/([^/]+)\/versions$/u); const one = path.match(/^\/api\/orchestration\/agents\/([^/]+)$/u);
      if (versions) { const definitionId = decodeURIComponent(versions[1]!); if (request.method === "GET") return response({ data: this.service.versions(actor.actorId, definitionId) }); if (request.method === "POST") { const body = versionSchema.parse(await request.json()); return response({ data: this.service.createVersion(actor.actorId, definitionId, { parentVersionId: body.parentVersionId, parentFingerprint: body.parentFingerprint, payload: body.payload ?? null }) }, 201); } }
