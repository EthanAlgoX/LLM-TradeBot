...
export class StrategyWorkbenchService {
  constructor(private readonly db: DatabaseSync, private readonly agents: AgentDefinitionService) {
...
CREATE TRIGGER IF NOT EXISTS strategy_workbench_drafts_no_update BEFORE UPDATE ON strategy_workbench_drafts BEGIN SELECT RAISE(ABORT, 'WORKBENCH_IMMUTABLE'); END;`);
  ...
  recommend(actorId: string, input: unknown) {
    ...
    if (missing.length) return { kind: "clarification" as const, intent, clarificationQuestions: missing.slice(0, 3).map((field) => ({ market: "交易市场或标的范围是什么？", horizon: "需要短、中、长哪些观察周期？", objective: "目标是趋势、收益还是回撤约束？", riskPreference: "可接受的风险偏好是什么？" })[field]), runtimeApplied: false, paperOnly: true, exchangeWriteAllowed: false };
    ...
    return { kind: "recommendation" as const, intent, recommendation };
  }
  ...
  apply(actorId: string, input: unknown) {
    ...
    const draft = StrategyDraftSchema.parse({ ...candidate, fingerprint: hash(candidate) }); this.db.prepare("INSERT INTO strategy_workbench_drafts VALUES (?, ?, ?, ?, ?, ?)").run(draft.draftId, actorId, recommendation.recommendationId, hash(body), JSON.stringify(draft), draft.createdAt); return draft;
  }
}
