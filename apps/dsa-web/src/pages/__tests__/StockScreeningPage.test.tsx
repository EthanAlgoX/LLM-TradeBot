...
vi.mock('../../components/strategy', () => ({
  StrategyTaskRunHistory: () => <section aria-label="运行记录">运行记录</section>,
  StrategyProductSelector: ({ onChange }: { onChange: (value: unknown) => void }) => {
    useEffect(() => {
      onChange({
        summary: { id: 8, name: '正式选股策略' },
        version: {
          id: 18,
          strategyId: 8,
          status: 'PUBLISHED',
          versionNumber: 2,
          strategyPurpose: 'candidate_screening',
        ...
        <button
          ...
          onClick={() => onChange({
            ...
            version: {
              ...
              versionNumber: 1,
              strategyPurpose: 'candidate_screening',
              screeningPolicy: { market: 'cn', strategy: 'capital_heat', maxCandidates: 5 },
            },
          })}
        >
          切换正式选股策略
  ...
  StrategyReportHeader: ({ title }: { title: string }) => <div>{title}</div>,
}));
...
describe('StockScreeningPage', () => {
  ...
  it('keeps implementation attribution and repeated guidance off the operation page', async () => {
    ...
    expect(document.title).toBe('选股扫描 - LLM TradeBot');
  ...
  it('renders hotspot details as user-facing Chinese without provider internals', async () => {
    ...
    expect(await screen.findByRole('heading', { name: '选股扫描' })).toBeInTheDocument();
    fireEvent.click(screen.getByRole('button', { name: /展开热点题材/ }));
  ...
  it('does not expose a second custom strategy editor on the product page', async () => {
    getScreeningStatus.mockResolvedValueOnce({
      enabled: true,
      available: true,
    });
    ...
    expect(await screen.findByText('选股已开启')).toBeInTheDocument();
    expect(screen.queryByLabelText('自定义策略 ID')).not.toBeInTheDocument();
    expect(screen.getByText('Dual Low')).toBeInTheDocument();

    fireEvent.click(screen.getByRole('button', { name: /运行选股/ }));
    await waitFor(() => expect(screenStocks).toHaveBeenCalledTimes(1));
    expect(screenStocks).toHaveBeenCalledWith({ market: 'cn', strategy: 'dual_low', maxResults: 3, strategyVersionId: 18 });
  });

  it('reads market, screening rule and candidate count from the selected StrategyVersion', async () => {
    ...
    expect(screen.getByText('双低')).toBeInTheDocument();
    expect(screen.getByText('3 只')).toBeInTheDocument();
    expect(screen.queryByRole('combobox', { name: '策略' })).not.toBeInTheDocument();
  ...
  it('clears previous screening candidates when strategy changes', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '切换正式选股策略' }));
