...
describe('HomePage', () => {
  ...
  it('requires a published research-report strategy on the standalone research page', async () => {
    vi.mocked(historyApi.getList).mockResolvedValue({
      total: 1,
      page: 1,
      limit: 20,
    ...
    vi.mocked(historyApi.getDetail).mockResolvedValue(historyReport);
    const listStrategies = vi.spyOn(strategyWorkspaceApi, 'listStrategies').mockResolvedValue([]);
    ...
    expect(await screen.findByRole('heading', { name: '单股研究' })).toBeInTheDocument();
    await waitFor(() => expect(document.title).toBe('单股研究 - LLM TradeBot'));
