...
const HomePage: React.FC = () => {
  ...
  const isStandaloneResearchTool = location.pathname === '/stock-research';
  ...
  const [researchReportId, setResearchReportId] = useState<number | null>(null);
  const [researchRunRequested, setResearchRunRequested] = useState(false);
  ...
  const researchExpectedStockCodeRef = useRef('');
  const researchTaskIdRef = useRef('');
  ...
  const handleProductStrategyChange = useCallback((selection: SelectedProductStrategy | null) => {
    ...
    setResearchRunRequested(false);
    researchExpectedStockCodeRef.current = '';
    researchTaskIdRef.current = '';
  }, []);
  ...
  useEffect(() => {
    document.title = isStandaloneResearchTool
      ? '单股研究 - LLM TradeBot'
      : t('home.pageTitle');
  }, [isStandaloneResearchTool, t]);
  ...
  const hasBoundResearchReport = Boolean(
    isStandaloneResearchTool
    && productStrategy
    && researchReportId !== null
    && selectedReport?.meta.id === researchReportId,
  );
  ...
  const handleCompletedTaskDataRefreshed = useCallback((task: TaskInfo) => {
    ...
    if (
      ...
      && task.taskId === researchTaskIdRef.current
      && getStockCodeKey(task.stockCode) === researchExpectedStockCodeRef.current
    ) {
      const completedReport = useStockPoolStore.getState().selectedReport;
      if (
        completedReport?.meta.id !== undefined
        && getStockCodeKey(completedReport.meta.stockCode) === researchExpectedStockCodeRef.current
      ) {
        setResearchReportId(completedReport.meta.id);
        setResearchRunRequested(false);
        researchTaskIdRef.current = '';
      }
    }
    setTodayAnalysisRefreshVersion((version) => version + 1);
  }, [isStandaloneResearchTool, researchRunRequested]);
  ...
  const handleSubmitAnalysis = useCallback(
    async (
      ...
      if (isStandaloneResearchTool && !productStrategy) return null;
      const response = await submitAnalysis({
        stockCode,
        stockName,
        originalQuery: forceRefresh ? stockCode || query : query,
        selectionSource: selectionSource ?? 'manual',
        forceRefresh,
        skills: isStandaloneResearchTool ? undefined : analysisSkills ?? selectedAnalysisSkills,
        strategyVersionId: isStandaloneResearchTool ? productStrategy?.version.id : undefined,
      });
      if (isStandaloneResearchTool && response && 'taskId' in response) {
        researchExpectedStockCodeRef.current = getStockCodeKey(stockCode || query);
        researchTaskIdRef.current = response.taskId;
        setResearchReportId(null);
        setResearchRunRequested(true);
      }
      return response;
    },
    [isStandaloneResearchTool, productStrategy, query, selectedAnalysisSkills, submitAnalysis],
  );
  ...
  useEffect(() => {
    ...
    if (state?.autoAnalyze) {
      void handleSubmitAnalysis(stockCode, stockName || undefined, 'import', state.skills);
    }
  ...
  const handleReanalyze = useCallback(() => {
    if (
      ...
      || (isStandaloneResearchTool && !productStrategy)
    ) {
      return;
    }
    ...
    void handleSubmitAnalysis(
      ...
      'manual',
      isStandaloneResearchTool ? undefined : selectedAnalysisSkills,
      true,
    );
  }, [handleSubmitAnalysis, isStandaloneResearchTool, productStrategy, selectedAnalysisSkills, selectedReport]);
  ...
  return (
    ...
    >
      <div className="flex-1 flex flex-col min-h-0 min-w-0 max-w-full lg:max-w-6xl mx-auto w-full">
        {isStandaloneResearchTool ? (
          <div className="mx-3 mt-3 space-y-4 md:mx-4">
            <section className="border-b border-border/70 pb-4" aria-labelledby="stock-research-title">
              <p className="text-xs font-medium text-primary">策略产品入口</p>
              <h1 id="stock-research-title" className="mt-1 text-2xl font-semibold tracking-[-0.02em] text-foreground">单股研究</h1>
              <p className="mt-1 max-w-3xl text-sm leading-6 text-secondary-text">选择策略中心正式发布的研究报告策略，再输入一只股票生成完整证据报告。报告不进入交易执行，也不代表策略已经回测验证。</p>
            </section>
            <StrategyProductSelector purpose="research_report" disabled={isAnalyzing} onChange={handleProductStrategyChange} />
            <p className="text-xs leading-5 text-secondary-text">
              当前通过正式 StrategyVersion 的统一 Python 内核入口调用成熟单股研究与报告链路；页面输入和报告归属都会绑定本次策略版本。
            </p>
          </div>
        ) : null}
        <header className="relative z-30 flex min-w-0 flex-shrink-0 items-center overflow-visible px-3 py-3 md:px-4 md:py-4">
          <div className="flex min-w-0 flex-1 flex-col gap-2.5 md:flex-row md:items-center">
            <div className="flex min-w-0 flex-1 items-center gap-2.5">
              {!isStandaloneResearchTool || hasBoundResearchReport ? (
                <button
              ...
              <div className="relative min-w-0 flex-1">
                <StockAutocomplete
                  ...
                  disabled={isAnalyzing || (isStandaloneResearchTool && !productStrategy)}
                  className={inputError ? 'border-danger/50' : undefined}
                />
              </div>
              {!isStandaloneResearchTool && analysisSkills.length > 0 ? (
            ...
            <div className="flex min-w-0 flex-wrap items-center gap-2.5">
              ...
              <button
                ...
                disabled={!query || isAnalyzing || (isStandaloneResearchTool && !productStrategy)}
        ...
        {setupNeedsAction && (!isStandaloneResearchTool || productStrategy) ? (
          <div className="px-3 pb-2 md:px-4">
            <InlineAlert
              variant="warning"
        ...
        <div className="flex-1 flex min-h-0 overflow-hidden">
          ...
          {sidebarOpen && (!isStandaloneResearchTool || hasBoundResearchReport) ? (
          ...
          >
            {!isStandaloneResearchTool ? <StrategyHealthPanel className="mb-3" /> : null}
            {marketReviewNotice ? (
              <div className="mb-3">
                <InlineAlert
            ...
            {isStandaloneResearchTool ? (
              <div className="mb-4">
                <StrategyTaskRunHistory
                  ...
                  refreshKey={`${researchTaskIdRef.current}-${researchRunRequested}`}
                  limit={3}
                />
              </div>
            ) : null}
            {isStandaloneResearchTool && !hasBoundResearchReport ? (
              <div className="flex h-full items-center justify-center py-8">
                <EmptyState
                  title={productStrategy ? "运行此策略生成新的单股报告" : "先选择一个正式单股研究策略"}
                  description={productStrategy
                    ? "输入一只股票开始兼容研究。本页只会把本次选择后新生成的报告放入当前工作区，旧历史报告不会被标记为这个版本的产出。"
                    : "选择后才会开放股票输入和分析；旧报告不会被冒充为当前策略版本的产出。"}
              ...
              <div className={isHistoryTrendOpen ? 'max-w-6xl space-y-4 pb-8' : 'max-w-4xl space-y-4 pb-8'}>
                {isStandaloneResearchTool && productStrategy ? (
                  <StrategyReportHeader
                    selection={productStrategy}
                    title={`${selectedReport.meta.stockName || selectedReport.meta.stockCode} · 单股研究报告`}
                    description="行情、新闻、基本面和模型判断继续使用同一份成熟报告结构，策略版本负责声明输入边界与 ResearchReport 输出契约。"
                    status="研究产出 · 非交易信号"
                  />
                ) : null}
                  ...
                  {!isMarketReviewHistoryReport ? (
                    <>
                      <Button
                        ...
                        size="sm"
                        disabled={isAnalyzing || selectedReport.meta.id === undefined || (isStandaloneResearchTool && !productStrategy)}
