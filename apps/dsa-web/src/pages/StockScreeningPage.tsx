...
import {
  StrategyProductSelector,
  StrategyReportHeader,
  StrategyTaskRunHistory,
  type SelectedProductStrategy,
...
const readPersistedScreenTask = (): PersistedScreenTask | null => {
  ...
  try {
    ...
    const restoredStrategyId = Number(parsed.strategyId);
    const restoredVersionId = Number(parsed.versionId);
    return {
      ...
      strategyId: Number.isInteger(restoredStrategyId) ? restoredStrategyId : undefined,
      versionId: Number.isInteger(restoredVersionId) ? restoredVersionId : undefined,
    };
  } catch {
    return null;
  }
...
const ReadOnlyRunFact: React.FC<{ label: string; value: string }> = ({ label, value }) => (
  <div className="min-w-0 border-b border-border/70 pb-3 lg:border-b-0 lg:pb-0">
...
const StockScreeningPage: React.FC = () => {
  ...
  const [productStrategy, setProductStrategy] = useState<SelectedProductStrategy | null>(null);
  const [resultProductStrategy, setResultProductStrategy] = useState<SelectedProductStrategy | null>(null);
  const [taskProductStrategyId, setTaskProductStrategyId] = useState<number | null>(restoredTask?.strategyId ?? null);
  const [taskProductVersionId, setTaskProductVersionId] = useState<number | null>(restoredTask?.versionId ?? null);
  ...
  const selectedProductVersionRef = useRef<number | null>(null);
  ...
  const handleProductStrategyChange = useCallback((selection: SelectedProductStrategy | null) => {
    setProductStrategy(selection);
    if (!selection && selectedProductVersionRef.current !== null) {
      clearPersistedScreenTask();
      selectedProductVersionRef.current = null;
      setActiveTaskId(null);
      setLoading(false);
      setCandidates([]);
      setScreenMeta(null);
      setExpandedCode(null);
      setResultProductStrategy(null);
    }
  }, []);
  ...
  useEffect(() => {
    document.title = '选股扫描 - LLM TradeBot';
  }, []);
  ...
  useEffect(() => {
    if (!productStrategy) return;
    const policy = productStrategy.version.screeningPolicy;
    const nextMarket = String(policy?.market || 'cn');
    const nextStrategy = String(policy?.strategy || 'dual_low');
    const nextMaxResults = Math.max(1, Math.min(100, Number(policy?.maxCandidates || 3)));
    const previousVersionId = selectedProductVersionRef.current;
    const versionChanged = previousVersionId !== null && previousVersionId !== productStrategy.version.id;
    const taskContextMatches = taskProductStrategyId === productStrategy.summary.id
      && taskProductVersionId === productStrategy.version.id;
    selectedProductVersionRef.current = productStrategy.version.id;

    if (activeTaskId && !taskContextMatches) {
      clearPersistedScreenTask();
      setActiveTaskId(null);
      setLoading(false);
      setTaskMessage('已切换正式策略，旧兼容选股任务不再显示在当前工作区。');
    }
    if (versionChanged || (!taskContextMatches && (activeTaskId || screenMeta))) {
      setCandidates([]);
      setScreenMeta(null);
      setExpandedCode(null);
      setResultProductStrategy(null);
    } else if (taskContextMatches) {
      setResultProductStrategy(productStrategy);
    }
    setMarket(nextMarket);
    setStrategy(nextStrategy);
    setMaxResults(nextMaxResults);
  }, [activeTaskId, productStrategy, screenMeta, taskProductStrategyId, taskProductVersionId]);
  ...
  const loadStrategies = useCallback(async () => {
    try {
      ...
      setStrategies([]);
      setError(err instanceof Error ? err.message : '底层筛选规则列表加载失败');
    }
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const task = await screeningApi.startScreen({ market, strategy, maxResults, strategyVersionId: productStrategy.version.id });
      persistScreenTask({
        taskId: task.taskId,
        market,
        strategy,
        maxResults,
        strategyId: productStrategy.summary.id,
        versionId: productStrategy.version.id,
      });
      setTaskProductStrategyId(productStrategy.summary.id);
      setTaskProductVersionId(productStrategy.version.id);
      setResultProductStrategy(productStrategy);
  ...
  return (
    <AppPage className="max-w-6xl space-y-6 pb-12 pt-6">
      <div className="flex flex-col gap-4 border-b border-border/70 pb-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-xs font-medium text-primary">策略产品入口</p>
          <h1 className="mt-1 text-2xl font-semibold tracking-[-0.02em] text-foreground">选股扫描</h1>
          <p className="mt-1 max-w-3xl text-sm leading-6 text-secondary-text">选择策略中心正式发布的选股策略，按版本冻结的市场、数据与筛选规则生成候选报告。候选结果不是订单，也不代表已经交易。</p>
        </div>
      ...
      <StrategyProductSelector purpose="candidate_screening" disabled={loading} onChange={handleProductStrategyChange} />
      ...
      <section className="workspace-surface p-5">
        <div className="mb-4 flex items-start justify-between gap-3">
          ...
          <span className="rounded-md border border-border bg-subtle px-2.5 py-1 text-xs font-medium text-secondary-text">
            {productStrategy ? `CandidateList · V${productStrategy.version.versionNumber ?? '—'}` : '等待选择策略'}
          </span>
        </div>

        <div className="grid gap-4 lg:grid-cols-[1fr_1.2fr_180px_auto] lg:items-end">
          <ReadOnlyRunFact label="市场" value={MARKETS.find((item) => item.id === market)?.label || market.toUpperCase()} />
          <ReadOnlyRunFact label="策略内筛选规则" value={selectedStrategy?.name || selectedStrategy?.title || strategy || '—'} />
          <ReadOnlyRunFact label="候选数量" value={`${maxResults} 只`} />

          <Button
            className="h-11 min-w-40"
            isLoading={loading}
            loadingText="筛选中..."
            disabled={!isScreeningEnabled || loading || !strategy.trim() || !productStrategy}
        ...
        <div className="mt-4 border-t border-border/70 pt-3 text-xs leading-5 text-secondary-text">
          {productStrategy
            ? '这些参数来自正式 StrategyVersion，在本页只读。当前通过统一 Python 内核入口调用成熟筛选与候选深度报告链路。'
            : '请先从上方选择一个正式选股策略。'}
        </div>
      </section>

      <StrategyTaskRunHistory
        selection={productStrategy}
        kind="candidate_screening"
        refreshKey={`${activeTaskId || "idle"}-${taskProgress}`}
      />
      ...
      {screenMeta ? (
        <section className="rounded-2xl border border-border bg-card/95 p-4 shadow-soft-card">
          {resultProductStrategy ? (
            <div className="mb-5">
              <StrategyReportHeader
                selection={resultProductStrategy}
                title="选股扫描报告"
                description="这是本次正式策略入口绑定的兼容选股任务结果；候选列表、排序依据、风险提示与单股深度信息使用统一报告层级。"
                status={`${candidates.length} 条候选 · 非交易指令`}
              />
            </div>
          ) : null}
