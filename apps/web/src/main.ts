...
function renderOrchestration(): string {
  ...
  const turns = real.map((turn, index) => {
    ...
    const provenance = recommendation?.provenance as { adapterMode?: string; modelConnectionRef?: string; catalogSnapshotFingerprint?: string } | undefined;
    const provenanceText = provenance
      ? `${escapeHtml(provenance.adapterMode ?? "PROVENANCE_UNAVAILABLE")} · ${escapeHtml(provenance.modelConnectionRef ?? "PROVENANCE_UNAVAILABLE")} · ${escapeHtml(provenance.catalogSnapshotFingerprint ?? "PROVENANCE_UNAVAILABLE")} · FALLBACK=false`
