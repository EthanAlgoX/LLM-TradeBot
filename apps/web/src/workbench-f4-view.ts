...
export type WorkbenchF4RunningAction = "preflight" | "backtest" | "walk-forward";
...
/**
 * Render server-projected F4 facts plus an ephemeral, version-scoped action
 * presentation. The running marker is not evidence authority: it exists only
 * until the POST returns its exact immutable-version projection.
 */
export function renderWorkbenchF4Evidence(input: unknown, locale: "zh-CN" | "en" | "en-US", runningAction?: WorkbenchF4RunningAction): string {
