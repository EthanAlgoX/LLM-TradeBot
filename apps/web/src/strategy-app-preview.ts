...
export function renderAgentCenter(context: StrategyPreviewRenderContext): string {
  ...
  const active = context.selectedRealAgent;
  const prompt = active?.version.payload.userInstructionPrompt ?? t(locale, "解释已标准化事实。", "Explain normalized facts.");
  const details = active ? `<section class="agent-real-detail" aria-live="polite"><span>SERVER AUTHORITY · ${esc(active.definition.definitionId)}</span><h2>${esc(active.version.payload.name)} · v${active.version.versionIndex}</h2><dl><div><dt>Lifecycle</dt><dd>${esc(active.lifecycle?.status ?? "draft")}</dd></div><div><dt>Data / upstream</dt><dd>${esc(active.version.payload.dataRef ?? (active.version.payload.upstreamArtifactSchemaRefs.join(", ") || "—"))}</dd></div><div><dt>Model</dt><dd>${esc(active.version.payload.modelRef ?? "deterministic")}</dd></div><div><dt>Version ID</dt><dd><code>${esc(active.version.versionId)}</code></dd><div><dt>Fingerprint</dt><dd><code>${esc(active.version.fingerprint)}</code></dd>${active.definition.sourceLineage ? `<div><dt>Source lineage</dt><dd><code>${esc(active.definition.sourceLineage.versionId)}</code></dd></div>` : ""}</dl><div class="agent-governance-actions"><button type="button" class="secondary-action" data-agent-validate ${active.lifecycle?.status !== "draft" ? "disabled" : ""}>Validate</button><button type="button" class="secondary-action" data-agent-publish ${active.lifecycle?.status !== "validated" ? "disabled" : ""}>Publish</button><button type="button" class="secondary-action" data-agent-clone>Clone</button><button type="button" class="secondary-action" data-agent-archive ${!["validated", "published"].includes(active.lifecycle?.status ?? "draft") ? "disabled" : ""}>Archive</button><button type="button" class="secondary-action" data-agent-diff ${context.agentVersions && context.agentVersions.length > 1 ? "" : "disabled"}>Diff v1/v2</button><button type="button" class="primary-action" data-agent-test>Run registered fixture</button></div><div data-agent-result></div><h3>${t(locale, "版本历史", "Version history")}</h3><ol class="agent-version-history">${(context.agentVersions ?? []).map((version) => `<li><strong>v${version.versionIndex}</strong><code>${esc(version.versionId)}</code><small>${esc(version.fingerprint)}${version.parentVersionId ? ` · parent ${esc(version.parentVersionId)}` : " · root"}</small></li>`).join("")}</ol></section>` : "";
  return `${previewHeader(locale, "AGENT CENTER · VERSIONED CONFIGURATION", t(locale, "Agent 中心", "Agent Center"), t(locale, "真实服务端版本事实；编排工作台只能引用已登记版本。", "Server-authoritative version facts; the workbench may reference registered versions only."))}
    ...
    <section class="agent-center-layout"><div class="agent-catalog"><span>REAL · SERVER AUTHORITY</span>${real.map((item) => `<button type="button" class="preview-agent-card" data-real-agent="${esc(item.definition.definitionId)}"><strong>${esc(item.version.payload.name)}</strong><span>v${item.version.versionIndex} · ${esc(item.definition.category)} · ${esc(item.lifecycle?.status ?? "draft")}</span><small>${esc((item.version.payload.dataRef ?? item.version.payload.upstreamArtifactSchemaRefs.join(", ")) || "—")} · ${esc(item.version.payload.modelRef ?? "deterministic")}</small><em>${esc(item.version.fingerprint)}</em></button>`).join("") || `<p class="preview-empty">${t(locale, "尚无真实 Agent。", "No real Agents yet.")}</p>`}</div><aside class="agent-detail"><span>${active ? "EDIT · NEW IMMUTABLE VERSION" : "CREATE · REAL VERSION"}</span><h2>${t(locale, "受控配置", "Controlled configuration")}</h2><p>${t(locale, "只编辑行为层；平台规则、工具权限和输出 Schema 已锁定。", "Edit behavior only; platform rules, tool permissions and output schema are locked.")}</p><label><span>Bearer token</span><input data-agent-token type="password" value="${esc(context.agentCenterToken ?? "")}"></label><label><span>${t(locale, "名称", "Name")}</span><input data-agent-name value="${esc(active?.version.payload.name ?? `${agentCategory} Agent`)}"></label><label><span>${t(locale, "用户 Prompt", "User prompt")}</span><textarea data-agent-prompt>${esc(prompt)}</textarea></label>${active ? `<button type="button" class="primary-action" data-save-agent-v2 ${active.lifecycle?.status === "draft" || active.lifecycle?.status === "validated" ? "" : "disabled"}>${t(locale, "基于精确父版本保存 v2", "Save v2 from exact parent")}</button>` : `<button type="button" class="primary-action" data-create-real-agent>Create ${esc(agentCategory)} v1</button>`}<dl><div><dt>System Prompt</dt><dd>LOCKED · reference only</dd></div><div><dt>Runtime</dt><dd>Paper Only · runtimeApplied=false · exchangeWriteAllowed=false</dd></div><div><dt>Test bench</dt><dd>DETERMINISTIC_TEST_ADAPTER · registered fixture only</dd></div></dl>${details}</aside></section>`;
}
