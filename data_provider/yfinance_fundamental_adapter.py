    ...
    def get_fundamental_bundle(self, stock_code: str) -> Dict[str, Any]:
        ...
        if div_series is not None and not div_series.empty:
            ...
            try:
                ...
                # The TTM window is inclusive by calendar date.  Using the current
                # wall-clock time silently drops an event exactly 365 calendar days
                # ago when its normalized midnight timestamp is earlier today.
                # Dividend history is date based, so normalize both sides first.
                cutoff = (pd.Timestamp.now(tz=div_series.index.tz) - pd.Timedelta(days=365)).normalize()
                ...
                for item in events:
                    try:
                        event_ts = pd.Timestamp(item["event_date"]).tz_localize(div_series.index.tz).normalize()
                    except Exception:
                        continue
                    if event_ts >= cutoff:
